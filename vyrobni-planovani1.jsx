...
export default function App() {
  ...
  const onCellDrop = (e, machine, date, slot) => {
    ...
    const slotsLeft = TOTAL_SLOTS - slot; // how many slots remain in this day from drop point
    const willOverflow = job.durationSlots > slotsLeft;

    if (willOverflow) {
      // ── SPLIT across midnight ──────────────────────────────────────────────
      // Check that part1 (today) slots are free
      ...
      if (source === "plan") {
        setJobs(prev => {
          ...
          // Do NOT merge here – we just split intentionally
      ...
      // ── NORMAL move (fits within one day) ─────────────────────────────────
  ...
  // Half-hour slot row labels (only show on :00)
  const slotLabel = (s) => s % 2 === 0 ? `${String(s/2).padStart(2,"0")}` : "";
  ...
  return (
    <div style={{ display:"flex", height:"100vh", overflow:"hidden", fontFamily:"'Inter','Segoe UI',system-ui,sans-serif", background:T.bg, color:T.text, fontSize:12 }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", minWidth:0 }}>
        ...
        {/* ── GRID: date sticky left, time column, machines as columns, days scroll down ── */}
                        ...
                        >
                          ...
                          <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"flex-start", padding:"3px 6px", overflow:"hidden", minWidth:0 }}>
                            <div style={{ display:"flex", alignItems:"center", gap:4, marginBottom:1 }}>
                              <span style={{ fontSize:10, fontWeight:700, color:jText, lineHeight:1.2, opacity:isPast?.6:1, overflow:"hidden", wordBreak:"break-word" }}>
      ...
      {editJob && (
        <div className="modal-bg" onClick={()=>setEditJob(null)}>
          <div onClick={e=>e.stopPropagation()} style={{
            width: 480, maxHeight:"92vh", display:"flex", flexDirection:"column",
            background:T.surface, borderRadius:14, overflow:"hidden",
            border:`1px solid ${T.border2}`, boxShadow:"0 24px 80px rgba(0,0,0,.7)",
          }}>

            {/* ── Header ── */}
            <div style={{ padding:"18px 20px 14px", borderBottom:`1px solid ${T.border}`, flexShrink:0, background:"linear-gradient(135deg,#13161f 0%,#0f1118 100%)" }}>
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:6 }}>
                <div style={{ width:32, height:32, background:`linear-gradient(135deg,${typeColor(editJob.type)},${typeLightColor(editJob.type)})`, borderRadius:9, display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, flexShrink:0 }}>
                  {editJob.type==="ok"?"📋":editJob.type==="reservation"?"📌":"🔧"}
                </div>
                <div>
                  <div style={{ fontSize:15, fontWeight:800, color:"#fff", lineHeight:1 }}>Editovat záznam</div>
                  <div style={{ fontSize:9, color:typeColor(editJob.type), fontWeight:700, letterSpacing:1.5, textTransform:"uppercase", marginTop:3 }}>
                    {JOB_TYPES.find(t=>t.value===editJob.type)?.label}
                  </div>
                </div>
                <button onClick={()=>setEditJob(null)} style={{ marginLeft:"auto", background:"transparent", border:"none", color:T.muted, fontSize:18, cursor:"pointer", lineHeight:1, padding:"2px 6px", borderRadius:6 }}>✕</button>
              </div>

              {/* Live náhled bloku */}
              <div style={{ background:`linear-gradient(135deg,${typeColor(editJob.type)} 0%,${typeLightColor(editJob.type)} 100%)`, borderRadius:8, padding:"8px 12px", display:"flex", gap:10, alignItems:"center", border:`1px solid ${typeLightColor(editJob.type)}55`, boxShadow:`0 4px 16px ${typeColor(editJob.type)}44` }}>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontSize:12, fontWeight:700, color:"#fff", lineHeight:1.2 }}>{editJob.orderNum||"—"}</div>
                  <div style={{ fontSize:9, color:"rgba(255,255,255,.75)", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", marginTop:2 }}>{editJob.description||"—"}</div>
                  <div style={{ fontSize:9, color:"rgba(255,255,255,.55)", marginTop:3 }}>
                    {editJob.date} · {String(Math.floor(editJob.slot/2)).padStart(2,"0")}:{editJob.slot%2===0?"00":"30"} · {MACHINES.find(m=>m===editJob.machine)||editJob.machine}
                  </div>
                </div>
                <div style={{ background:"rgba(0,0,0,.25)", borderRadius:6, padding:"4px 10px", textAlign:"center", flexShrink:0 }}>
                  <div style={{ fontSize:16, fontWeight:800, color:"rgba(255,255,255,.85)", lineHeight:1 }}>{editJob.durationSlots%2===0?editJob.durationSlots/2:Math.floor(editJob.durationSlots/2)}</div>
                  <div style={{ fontSize:7, color:"rgba(255,255,255,.45)", letterSpacing:.5 }}>{editJob.durationSlots%2===0?"HOD":":30"}</div>
                </div>
              </div>
            </div>
            ...
            <div style={{ overflowY:"auto", flex:1 }}>
              ...
              <div style={{ padding:"14px 20px 12px", borderBottom:`1px solid ${T.border}` }}>
                ...
                <div style={{ display:"flex", gap:6 }}>
                  {JOB_TYPES.map(t => (
                    <button key={t.value}
                      onClick={()=>setEditJob(p=>({...p,type:t.value,color:typeColor(t.value)}))}
                      style={{ flex:1, padding:"8px 4px", borderRadius:8, border:`1px solid ${editJob.type===t.value?t.color:T.border2}`, background:editJob.type===t.value?`${t.color}28`:T.surface2, color:editJob.type===t.value?t.color:"#6b7894", fontSize:9, fontWeight:700, cursor:"pointer", textAlign:"center", lineHeight:1.4, transition:"all .15s" }}>
                      <div style={{ fontSize:14, marginBottom:2 }}>{t.value==="ok"?"📋":t.value==="reservation"?"📌":"🔧"}</div>
                      {t.label.split("(")[0].trim()}
                    </button>
                  ))}
                </div>
              ...
              <div style={{ padding:"14px 20px 12px", borderBottom:`1px solid ${T.border}` }}>
                <div style={{ fontSize:10, fontWeight:700, color:"#9ba8c0", textTransform:"uppercase", letterSpacing:1.2, marginBottom:10 }}>
                  {editJob.type==="maintenance"?"Detail údržby":"Zakázka"}
                </div>
                <div className="fr">
                ...
                <div className="fc" style={{ marginBottom:0 }}>
                  <label>Popis</label>
                  <textarea rows={2} value={editJob.description||""} onChange={e=>setEditJob(p=>({...p,description:e.target.value}))} />
                </div>
              </div>

              {/* Sekce: Plánování */}
              <div style={{ padding:"14px 20px 12px", borderBottom:`1px solid ${T.border}` }}>
                <div style={{ fontSize:10, fontWeight:700, color:"#9ba8c0", textTransform:"uppercase", letterSpacing:1.2, marginBottom:10 }}>Plánování</div>
                <div className="fr">
                  <div><label>Datum (dd.mm.rrrr)</label><input type="text" value={editJob.date||""} onChange={e=>setEditJob(p=>({...p,date:e.target.value}))} placeholder="19.02.2026" /></div>
                  <div>
              ...
              {/* Sekce: Termíny (skrytá pro údržbu) */}
              {editJob.type !== "maintenance" && (
                <div style={{ padding:"14px 20px 12px", borderBottom:`1px solid ${T.border}` }}>
                  <div style={{ fontSize:10, fontWeight:700, color:"#9ba8c0", textTransform:"uppercase", letterSpacing:1.2, marginBottom:10 }}>Termíny</div>
                  <div className="fr">
                  ...
                  <div className="fr" style={{ marginBottom:0 }}>
                    <div><label>🚚 Expedice – termín</label><input type="date" value={editJob.exp||""} onChange={e=>setEditJob(p=>({...p,exp:e.target.value}))} style={{ colorScheme:"dark" }} /></div>
                    <div><label>Badge</label><input type="text" value={editJob.badge||""} onChange={e=>setEditJob(p=>({...p,badge:e.target.value}))} placeholder="SCH Lumina LED" /></div>
                  </div>
                </div>
              )}
              ...
              {editJob.type !== "maintenance" && (
                <div style={{ padding:"14px 20px 12px", borderBottom:`1px solid ${T.border}` }}>
                  <div style={{ fontSize:10, fontWeight:700, color:"#9ba8c0", textTransform:"uppercase", letterSpacing:1.2, marginBottom:10 }}>Potvrzení</div>
                  <div style={{ display:"flex", gap:12 }}>
                    <label style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", flex:1, background:editJob.dataOk?"rgba(74,222,128,.08)":T.surface2, border:`1px solid ${editJob.dataOk?"rgba(74,222,128,.4)":T.border2}`, borderRadius:8, padding:"10px 12px", transition:"all .15s" }}>
                      <input type="checkbox" checked={!!editJob.dataOk} onChange={e=>setEditJob(p=>({...p,dataOk:e.target.checked}))} style={{ width:14, height:14, accentColor:"#4ade80" }} />
                      <span style={{ fontSize:11, fontWeight:600, color:editJob.dataOk?"#4ade80":T.muted }}>📥 Data OK</span>
                    </label>
                    <label style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", flex:1, background:editJob.matOk?"rgba(74,222,128,.08)":T.surface2, border:`1px solid ${editJob.matOk?"rgba(74,222,128,.4)":T.border2}`, borderRadius:8, padding:"10px 12px", transition:"all .15s" }}>
                      <input type="checkbox" checked={!!editJob.matOk} onChange={e=>setEditJob(p=>({...p,matOk:e.target.checked}))} style={{ width:14, height:14, accentColor:"#4ade80" }} />
                      <span style={{ fontSize:11, fontWeight:600, color:editJob.matOk?"#4ade80":T.muted }}>📦 Materiál OK</span>
                    </label>
                  </div>
                </div>
              )}
            ...
            {/* ── Footer: akce ── */}
            <div style={{ padding:"14px 20px", borderTop:`1px solid ${T.border}`, flexShrink:0, background:T.surface, display:"flex", gap:8 }}>
              <button onClick={saveEdit}
                style={{ flex:1, padding:"11px", borderRadius:8, border:"none", background:T.accent, color:"#000", fontSize:12, fontWeight:800, cursor:"pointer", letterSpacing:.3 }}>
                ✓ Uložit změny
              </button>
              <button onClick={()=>setEditJob(null)}
                style={{ padding:"11px 16px", borderRadius:8, border:`1px solid ${T.border2}`, background:T.surface2, color:T.muted, fontSize:12, fontWeight:600, cursor:"pointer" }}>
                Zrušit
              </button>
              <button onClick={()=>deleteJob(editJob.id)}
                style={{ padding:"11px 14px", borderRadius:8, border:"1px solid rgba(239,68,68,.3)", background:"rgba(127,29,29,.4)", color:"#fca5a5", fontSize:12, fontWeight:700, cursor:"pointer" }}>
                🗑 Smazat
              </button>
            </div>
