...
test("SPLIT_SHARED_FIELDS: má přesně 32 položek (tripwire — nové sdílené pole se přidává vědomě)", () => {
  assert.equal(SPLIT_SHARED_FIELDS.length, 32);
  assert.equal(new Set(SPLIT_SHARED_FIELDS).size, 32, "duplicita v seznamu");
});
