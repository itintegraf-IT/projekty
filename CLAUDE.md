...
`--experimental-test-module-mocks` je nutný kvůli `sessionVersion.test.ts` a `cascadeLimit.server.test.ts` (oba `mock.module`); ostatní ho nepotřebují. (`scheduleSlotFinder.server.test.ts` ho potřeboval do 8. 8. 2026 — od zapojení revizí si finder bere Prisma klienta povinným parametrem, takže si test podstrkuje fake přímo, bez mockování modulu.)
...
**ZAKÁZKA na chain pushi horizont NEMÁ** samostatně — `computeChainPush`/`computeChainPushAttempt` (`src/lib/overlapResolver.ts`, volané z `resolveChainPushFromDb` v `overlapResolver.server.ts`) mají komentář „Zakázka horizont nemá, u ní je to skutečné selhání" (dnes cca řádek 161) — kontrast s konstantou `MAX_RIGID_PUSH_MS` = 7 dní (dnes cca řádek 39), která platí jen pro REZERVACI/UDRZBU. Havárie 17. 8. 2026 16:31 vznikla přesně tak — posun konce o délku noční pauzy odsunul 88 navazujících zakázek, některé o týdny dál, protože nic nezastavilo, jak daleko chain push smí zajít.

**Náprava (etapa „autoposun viditelný a vratný"): strop + potvrzení odvozené od DŮSLEDKU posunu, ne od velikosti gesta.** `CASCADE_CONFIRM_MAX_BLOCKS = 5` bloků **nebo** posun dál než `MAX_RIGID_PUSH_MS` (7 dní) — `measureCascade`/`assertCascadeConfirmed` (`src/lib/cascadeLimit.ts`/`cascadeLimit.server.ts`) měří **NEJVĚTŠÍ posun JEDNOHO bloku** v dávce, nikdy rozpětí celé dávky (dlouhá, ale drobná kaskáda by jinak vyšla stejně jako jediný blok odsunutý o měsíc). Vynucuje se v `resolveChainPushFromDb`, tedy na jednom místě pro všech šest serverových zápisových cest (POST · PUT · batch · split · reflow bloku · reflow stroje). **Vynuceno od 21. 8. 2026** (`CASCADE_CONFIRM_ENFORCED = true`, rozhodnutí Vojty, etapa B4) — překročení prahu (5 bloků nebo posun dál než 7 dní) odroluje celou transakci a vrátí 409 `CASCADE_CONFIRM`; uživatel dopad potvrdí v dialogu a požadavek se zopakuje s `cascadeConfirmed: true`. Práh 5 je podložený měřením: log testovací instance ukázal, že běžné přetažení plánovače posouvá 3–4 bloky, práh 5 tedy sedí těsně nad běžnou prací. Když jedno gesto uživatele vyrobí několik chain-push volání (batch s víc kotvami, hromadný přepočet stroje), per-volání kontrola se **potlačí** (`skipCascadeCheck: true`) a autoritativní je **souhrn za celé gesto** (`path: "batch-total"`/`"reflow-machine"`) — jinak by uživatel odklepl menší dopad, než jaký se skutečně provede.

**Vypínač autoposunu (Task 6D, etapa „autoposun viditelný a vratný") je TVRDÁ pojistka vedle měkkého kaskádového dialogu výš — obě se doplňují.** Dialog výš se ptá, ale gesto nakonec projde; vypínač chain push VŮBEC NESPUSTÍ. Je to per-uživatelská preference `autoshift` (`on`/`off`, `PUT /api/me/preferences`, ne localStorage — Lukáš má mít stejné nastavení na každém počítači; localStorage je jen optimistická cache jako u `zoom`), výchozí **ZAPNUTO** (chování jako dnes). Platí na všech šesti serverových zápisových cest, které sahají na `resolveChainPushFromDb` (přímo, nebo přes `reflowBlockInTx` u obou reflow cest) — POST/PUT/batch čtou `resolveChain === true` (klient ho posílá vždycky výslovně), split a obě reflow cesty čtou `resolveChain !== false` (chybějící příznak = ZAPNUTO — do etapy 6 tam `resolveChain` v těle vůbec nebyl, takže `=== true` by starému klientovi tiše vypnulo chain push). Vypnuto ⇒ kolize s navazujícím blokem skončí běžným `OVERLAP`, jen s hláškou z `src/lib/autoShiftOff.ts` (`AUTOSHIFT_OFF_OVERLAP_MESSAGE`), která vysvětlí PROČ se to neposunulo samo — původní detail (číslo bloku, stroj) se připojí v závorce, ne zahodí. **Netýká se zámku pracovní doby** (`workingTimeLock`, tlačítko vedle v hlavičce) — to je snap VLASTNÍHO taženého bloku do pracovní doby, jiná funkce, jiný přepínač; neplést je. Strážný test `autoShiftWiring.test.ts` hlídá všech šest cest a kontroluje i MÍSTO POUŽITÍ (ne jen parsování) — samotné `const resolveChain = …` bez guardu, který chain push řídí, by testem neprošlo.

**Kontrola kaskády u editace směn (`PUT /api/machine-week-shifts`, od 17. 8. 2026) MĚŘÍ skutečný stav, nesimuluje ho.** Uvnitř JEDNÉ transakce zavolá `detectCalendarDrift` dvakrát — `driftBefore` PŘED upserty směn, `driftAfter` PO nich (tx vidí vlastní zápisy) — a rozdíl spočítá `classifyCascade` (`src/lib/cascadeCheck.ts`) na `newlyHomeless` (blokuje, 409) / `newlyLonger` (neblokuje, jen se pojmenuje v odpovědi). Bez `force` blokující kolize transakci ODROLUJE — směny se nezapíšou vůbec. **`force=1` nic neposouvá ani nepřepočítává** — přeskočí jen `if` podmínku, která by jinak hodila `CONFLICT`, a zapíše výhradně řádky `MachineWeekShifts`; `Block` se v žádné větvi PUT nemutuje. Dřívější kontrola (`findConflictingBlocks.ts`, zaniklo 17. 8. 2026 i s 11 testy) simulovala cílovou konfiguraci ručně a hlásila falešné poplachy, protože simulace neznala skutečnou expanzi tiskových hodin — nahrazeno měřením, protože „cílový stav se často nemusí simulovat, stačí ho změřit a transakci odrolovat".
...
- **U NEODLOŽENÝCH bloků musí klasifikovat shodně** — rozejití = štítek na zakázce tvrdí něco jiného než pruh nad strojem. Změna guardu na jedné straně se vždy promítá na druhou; hlídá to tabulkový test parity v `calendarDrift.server.test.ts` (obsahuje i vstupy, které si každá strana počítá sama — odstávky, zarovnání startu). Od etapy 9 obě strany posuzují i REZERVACE s `printMinutes` (klient přes `usesTiskoveHodiny` v `tryExpandForBlock`, server přes where filtr) — parity tabulka má REZERVACE fixtury.
- **Odložené bloky (`scheduleBypassed`) posuzuje JEN klient.** Server je vyřazuje už ve `where` a je to záměr: příznak je *spočítaná pravda* (`effectivelyBypassed = !conforms`), takže odložená zakázka je z definice nekonformní. Bez toho filtru by každé vědomé odložení trvale svítilo jako „nesedí na kalendář" — notifikace z každé úpravy směn, nikdy nenulový report, a hromadné „Přepočítat" by ji vystěhovalo bez šance to vzít zpět. Vlastní důvody `PARKED`/`STALE_BYPASS` proto na serveru NIKDY nevzniknou. Souhrnné počítadlo nad strojem (`driftCountByMachine` v `TimelineGrid.tsx`) je musí vynechávat taky, jinak slibuje akci, která u nich neproběhne.
...
- **Strop operací v jedné undo dávce** (`sanitizeUndoOps`) — nad ním 400 `VALIDATION_ERROR`. Číslo `UNDO_MAX_OPS` (`src/lib/undo/limits.ts`) je jediný zdroj pravdy pro obě strany (server i klient) — neopisovat natvrdo.
- **`buildReflowCommand` (`src/lib/undo/commands.ts`) záměrně NEvyžaduje, aby byl blok v klientském stavu** — chain push posouvá i bloky mimo načtený rozsah dní (klient má jen zobrazené dny) a `applyServerBlocks` je do stavu nepřidává, takže s liveness guardem (`requireLive`) by Ctrl+Z po přepočtu selhal vždycky. Liveness guard drží **jen** `buildMoveCommand` a ostatní buildery; sjednotit ho zpátky na všechny by rozbilo Ctrl+Z u přepočtu. Skutečnou kontrolu souběhu i tak dělá server — `expectedUpdatedAt` všech cílů se kontroluje atomicky před prvním zápisem (viz undo endpoint výš).
...
**Sdílené jádro:** `src/lib/errors.ts` (AppError/errorStatus) · `authz.ts`+`auth.ts` (requireRole) · `logger.ts` · `scheduleValidationServer.ts` (validateAndComputeEnd) · `printTime.ts`/`printTime.server.ts`/`printTimeClient.ts` (tiskové hodiny) · `blockPayload.ts` (Block→POST payload, jediný zdroj) · `blockStyles.ts` · `machines.ts` · `zLayers.ts` · `dateUtils.ts` · `plannerTypes.ts` · `uiStyles.ts` · `reflow.server.ts` · `calendarDrift.server.ts` · `cascadeCheck.ts` (`classifyCascade`/`computeConflictWindow` — diferenční kontrola kaskády, viz sekce Chain push výš) · `cascadeDialogText.ts` (texty kaskádového dialogu, `CASCADE_REASON_LABELS`) · `cascadeLimit.ts` (`measureCascade`/`CASCADE_CONFIRM_MAX_BLOCKS`/`CASCADE_CONFIRM_ENFORCED` — práh a dopad autoposunu, viz sekce Chain push výš) · `cascadeLimit.server.ts` (`assertCascadeConfirmed` — jediné místo, které práh vynucuje/loguje) · `cascadeResponse.ts` (`cascadeConfirmBody` — jednotné tělo 409 `CASCADE_CONFIRM` napříč šesti zápisovými cestami) · `cascadeConfirmClient.ts` (`fetchWithCascadeConfirm` — klientský retry po potvrzení dialogu) · `autoShiftOff.ts` (`autoShiftExplicitlyOff`/`overlapMessageFor`/`AUTOSHIFT_OFF_OVERLAP_MESSAGE` — vypínač autoposunu, viz sekce Chain push výš) · `monitorDriftMark.ts` (`shouldMarkDrift` — značka „nesedí na kalendář" na Monitoru) · `undoApply.server.ts` (atomické undo/redo, `sanitizeUndoOps`+`applyUndoOps`) · `splitSharedFields.ts` (`SPLIT_SHARED_FIELDS`, sdílené serverem i klientem) · `undo/restoreFields.ts` (`UNDO_RESTORABLE_FIELDS` allowlist) · `undo/splitSiblingFields.ts` (`buildSplitEditTargets`/`buildSplitEditTargetsWithShifted`/`buildPassiveSiblingTargets`) · `prismaTx.ts` (`PrismaTransactionClient`, jediný zdroj) · `orderSearch.ts` (`blockMatchesQuery` — co znamená shoda při hledání zakázky; sdílí hlavičkové hledání, ztlumení v `TimelineGrid`, tiskařský `OrderSearchSheet` i DTP přehled) · `overdueState.ts` (`isOverdueUnacknowledged` — zpožděná neodklepnutá zakázka je od 17. 8. 2026 VŽDY červená, žádná dvoustupňovost ani časové okno; velkou kartu Monitoru neřídí vůbec, ta drží zakázku, dokud tiskař nedá HOTOVO nebo „Přeskočit →" (`pickHeroBlock`)) · `dtpOverview.ts` (výběr a řazení zakázek v DTP přehledu) · `reflowBefore.server.ts` (`ReflowBeforeSnapshot` — poziční stav bloku PŘED autoposunem, serializovaný tvarem shodným s klientským `BlockSnapshot`, aby ho šlo poslat do `buildReflowCommand` beze změny) · `reflowToastText.ts` (`reflowMachineToast`/`reflowBlockToast` — hlášky po „Přepočítat", testovatelné odděleně od `PlannerPage.tsx`) · `undo/limits.ts` (`UNDO_MAX_OPS` — jediný zdroj pravdy pro strop dávky, klient i server).
