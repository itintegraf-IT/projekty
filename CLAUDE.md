...
- **`scheduleBypassed` z bloku mizí JEDINOU cestou — adresným tlačítkem „Přepočítat"** nad konkrétní zakázkou (`reflowBlockInTx`). Nepřidávat žádnou automatiku, která by značku rušila nebo blok přesouvala bez kliknutí uživatele (viz `docs/POUCENI.md` a „minimum automatiky bez vědomí plánovače“).
