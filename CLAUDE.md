...
**ZAKÁZKA na chain pushi horizont NEMÁ.** `computeChainPush`/`computeChainPushAttempt` (`src/lib/overlapResolver.ts`, volané z `resolveChainPushFromDb` v `overlapResolver.server.ts`) mají komentář „Zakázka horizont nemá, u ní je to skutečné selhání" (dnes cca řádek 161) — kontrast s konstantou `MAX_RIGID_PUSH_MS` = 7 dní (dnes cca řádek 39), která platí jen pro REZERVACI/UDRZBU. Tohle je otevřený dluh, ne popis hotového stavu: havárie 17. 8. 2026 16:31 vznikla přesně tak — posun konce o délku noční pauzy odsunul 88 navazujících zakázek, některé o týdny dál, protože nic nezastavilo, jak daleko chain push smí zajít. Diferenční kontrola v `PUT /api/machine-week-shifts` (níž) hlásí kaskádu PŘED uložením směn, ale samotný chain push při běžném přesunu/resize bloku (mimo editaci směn) žádný analogický strop nemá. Náprava (strop + potvrzení odvozené od DŮSLEDKU posunu, ne od velikosti gesta) je v backlogu, ne v této vlně.
