...
- **Velikosti v `/reporty`** vždy přes `src/lib/reportTokens.ts` (`reportTypeScale` s podlahou 10 px, `reportGlyph`, `reportRadius`), nikdy holé číslo — obdoba `plannerTypography.ts` pro planner a `uiStyles.ts` pro admin.
