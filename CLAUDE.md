...
**Na rozdíl od overlap guardu a chain pushe výš, kontrola kaskády NEPLATÍ pro všechny typy.** `detectCalendarDrift` měří jen `ZAKAZKA` (`where`: `type: "ZAKAZKA"`, `scheduleBypassed: false`, `printMinutes: { gt: 0 }`, `printCompletedAt: null`, plus zarovnaný start) — REZERVACE a ÚDRŽBA se tiše přeskakují, kontrola o nich nehlásí vůbec nic. Stará (smazaná) kontrola `findConflictingBlocks.ts` běžela BEZ filtru typu, takže hlásila i je; tohle je tedy zúžení rozsahu proti dřívějšímu stavu, ne nikdy nepokrytá oblast. U víkendové údržby nebo vypnuté sobotní směny se editace směn dnes neozve — a podle pravidla „Zdí… zůstává" výš se taková rigidní údržba mimo kalendář stává zdí pro chain push, takže příští drop vedle ní skončí 409 bez jakéhokoli varování napřed.
